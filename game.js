...
function Bear() {
  //speed of the bear is controlled here
  this.speedInput = document.getElementById("speedBear");
  this.dBear = this.speedInput.value;
...
function restart() {
  ...
  let i = 0;
  let nbBees = bees.length;
  while (i < nbBees) {
    bees[bees.length - 1].htmlElement.remove();
    bees.pop(); //add the bee object to the bees array
    i++;
  }
  //clearTimer(updateTimer);
  start();
